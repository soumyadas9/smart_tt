...
def clean_teacher_name(left: str) -> str:
    # remove trailing codes like "(/SS)" or "(PN)" safely
    s = left.strip()
    # drop anything in the last parentheses group if it looks like a code
    # (keeps name readable without breaking uniqueness)
...
def is_lecture_room(code: str) -> bool:
    # user rule: PURE numerics (including 010) are lecture rooms
    return code.isdigit()
...
def run_seed_master():
    ...
    with get_conn() as conn:
        # Branches: start with these (you can add more from UI later)
        ...
        # Rooms: split lecture vs lab rooms
        ...
        # Teachers + Subjects/Labs
        all_subjects = set()
        all_labs = set()

        for line in [x.strip() for x in TEACHER_BLOCK.splitlines() if x.strip()]:
            # handle both "-" and " - " and weird spacing
            if "-" not in line:
                # teacher with no subject list
            ...
            for tok in tokens:
                # LAB detection: anything containing "LAB" goes into labs table
                if "LAB" in tok.upper():
                    all_labs.add(tok)
                else:
                    all_subjects.add(tok)

        # Insert subjects (name + short)
        for s in sorted(all_subjects):
            # keep name=short for now (you can rename later via UI if you want)
            upsert_one(conn, "subjects", "name", s, extra_cols={"short": s})

        # Insert labs (name + short)
