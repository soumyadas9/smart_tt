...
# ==========================
# SESSION + CORS (CRITICAL)
# ==========================
app.secret_key = "super-secret-key-change-in-production"

# Cookie rules for localhost dev
app.config["SESSION_COOKIE_SAMESITE"] = "Lax"
app.config["SESSION_COOKIE_SECURE"] = False
app.config["SESSION_COOKIE_HTTPONLY"] = True

# Allow frontend origin + cookies
CORS(
    app,
...
@app.get("/")
def home():
    return {"message": "Backend running. Try /health"}
...
def _inject_teacher_shorts(branch_output, teacher_short_map):
    ...
    If teacherShort already exists (from per-batch mapping), do NOT overwrite it.
    """
...
def lock_slot():
    ...
    if not branch or not day:
        return {"error": "branch, day, period required"}, 400

    with get_conn() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO locked_slots (branch_name, day, period) VALUES (?,?,?)",
            (branch, day, period),
        )
        conn.commit()
    return {"ok": True}
...
def unlock_slot():
    ...
    with get_conn() as conn:
        conn.execute(
            "DELETE FROM locked_slots WHERE branch_name=? AND day=? AND period=?",
            (branch, day, period),
        )
        conn.commit()
    return {"ok": True}
...
@app.get("/locked-slots/<branch>")
@login_required
def get_locked(branch):
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT day, period FROM locked_slots WHERE branch_name=?",
            (branch,),
        ).fetchall()
...
# ==========================
# Branch Labs (default mapping) (Protected)
# ==========================
...
# ==========================
# Branch Lab Batches (batch-wise mapping) (Protected)
# ==========================
