...
def generate_labs_only():
    ...
    with get_conn() as conn:
        for bid in branch_ids:
            ...
            # merge teacher/room views across branches
...
def generate_full():
    ...
    # GLOBAL busy maps across ALL branches (hard constraints)
    ...
    with get_conn() as conn:
        for bid in branch_ids:
            ...
            for tname, grid in tv.items():
                teachers_accum.setdefault(tname, {d: {pp: [] for pp in range(1,9)} for d in ["Monday","Tuesday","Wednesday","Thursday","Friday"]})
                for day in grid:
                    for p in grid[day]:
                        teachers_accum[tname][day][p] += grid[day][p]

            for rcode, grid in rv.items():
                rooms_accum.setdefault(rcode, {d: {pp: [] for pp in range(1,9)} for d in ["Monday","Tuesday","Wednesday","Thursday","Friday"]})
                for day in grid:
                    for p in grid[day]:
                        rooms_accum[rcode][day][p] += grid[day][p]
